...
class IndicatorSettingsWindow:
    ...
    def open_indicator_settings(self, indicator):
        ...
        # # Now you can use settings_window.window to create widgets
        # for idx, param in enumerate(params):
        #     ttk.Label(settings_window.window, text=param).grid(row=idx, column=0, padx=10, pady=5, sticky=tk.W)  # Access settings_window.window
        #     if param == 'method':
        #         var = tk.StringVar(value=self.indicator_settings[indicator]['params'].get(param, 'EMA'))
        #         self.param_vars[param] = var
        #         combobox = ttk.Combobox(settings_window.window, textvariable=var, values=[e.name for e in Method])  # Access settings_window.window
        ...
        #         self.param_vars[param] = var
        #         combobox = ttk.Combobox(settings_window.window, textvariable=var, values=[e.name for e in AppliedPrice])  # Access settings_window.window
        #         combobox.grid(row=idx, column=1, padx=10, pady=5, sticky=tk.EW)
        #     else:
        #         var = tk.StringVar(value=self.indicator_settings[indicator]['params'].get(param, ''))
        #         self.param_vars[param] = var
        #         ttk.Entry(settings_window.window, textvariable=var).grid(row=idx, column=1, padx=10, pady=5, sticky=tk.EW)  # Access settings_window.window


        # # Add fields for RSI overbought and oversold levels
        # if indicator.lower() == 'rsi':
        #     ttk.Label(settings_window.window, text='overbought').grid(row=len(params), column=0, padx=10, pady=5, sticky=tk.W)
        #     overbought_var = tk.StringVar(value=self.indicator_settings[indicator]['params'].get('overbought', '70'))
        #     self.param_vars['overbought'] = overbought_var
        #     ttk.Entry(settings_window.window, textvariable=overbought_var).grid(row=len(params), column=1, padx=10, pady=5, sticky=tk.EW)

        #     ttk.Label(settings_window.window, text='oversold').grid(row=len(params) + 1, column=0, padx=10, pady=5, sticky=tk.W)
        #     oversold_var = tk.StringVar(value=self.indicator_settings[indicator]['params'].get('oversold', '30'))
        #     self.param_vars['oversold'] = oversold_var
        #     ttk.Entry(settings_window.window, textvariable=oversold_var).grid(row=len(params) + 1, column=1, padx=10, pady=5, sticky=tk.EW)


        # # Pass self and user_data_dir to IndicatorSpecificSettingsWindow
        # settings_window = IndicatorSpecificSettingsWindow(
        #     self.window, self, indicator, self.indicator_settings, params, self.user_data_dir
        # )
