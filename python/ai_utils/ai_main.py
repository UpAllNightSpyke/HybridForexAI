...
def main():
    ...
    settings_file = os.path.join(user_data_path, 'settings',
    ...
    # 2. Prepare Data
    normalized_data_dir = os.path.join(user_data_path, 'forex_data',
                                       'normalized')

    # Find the normalized data file
    for filename in os.listdir(normalized_data_dir):
        if filename.endswith("_normalized.csv"):
            normalized_data_file = os.path.join(normalized_data_dir,
                                                filename)
            break
    else:
    ...
    if match:
        ...
        raise ValueError(
            f"Could not extract symbol and timeframe from filename: {filename}"
        )
    ...
    algorithm_func = available_algorithms.get(selected_algorithm)
    ...
    algorithm_params.pop('module_path',
                         None)  # Remove module_path if it exists
    algorithm_params['learning_rate'] = float(
        algorithm_params.get('learning_rate', 0.0007))
